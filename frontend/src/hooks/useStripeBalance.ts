...
export function useStripeBalance() {
  return useQuery<StripeBalanceResult>({
    ...
    queryFn: async (): Promise<StripeBalanceResult> => {
      ...
      const usdAvailable = available.find((b: { currency: string; amount: number }) => b.currency === 'usd');
      const usdPending = pending.find((b: { currency: string; amount: number }) => b.currency === 'usd');
