...
export default function OwnerStripeConnectCard() {
  ...
  const [connectedAccount, setConnectedAccount] =
    React.useState<StripeConnectedAccount | null>(null);
  ...
  }, [storedAccountId, connectedAccount, retrieveAccount.mutate]);

  const handleCreateAccount = async () => {
    if (!email.trim()) {
      toast.error("Email required", {
        description: "Please enter an email address for the connected account.",
      });
      return;
    }
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                Connected account for receiving payouts
              </CardDescription>
            </div>
            {isActive ? (
        ...
        <CardContent className="space-y-4">
          ...
          {needsOnboarding && (
            <Alert>
              ...
              <AlertDescription>
                Your account setup is incomplete. Complete onboarding to enable
                payouts.
              </AlertDescription>
            </Alert>
          )}
          ...
          <div className="flex gap-2">
            ...
            >
              <RefreshCw
                className={`h-4 w-4 ${retrieveAccount.isPending ? "animate-spin" : ""}`}
              />
      ...
      <Card className="border-slate-200 dark:border-slate-700">
        ...
        <CardContent className="pt-2">
          <div className="flex items-center gap-2 text-muted-foreground">
            ...
            <span>
              Fetching account status for {STRIPE_CONNECT_ACCOUNT_ID}...
            </span>
          </div>
        </CardContent>
      </Card>
      ...
      <CardContent className="space-y-4">
        {retrieveAccount.isError && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              ...
              : {(retrieveAccount.error as any)?.message || "Unknown error"}.
              You can retry or create a new account below.
            </AlertDescription>
          </Alert>
        )}
        ...
        <div className="rounded-lg bg-slate-50 dark:bg-slate-900 border p-3 space-y-1">
          <p className="text-xs text-muted-foreground font-medium">
            Pre-configured Connect Account
          </p>
        ...
        <div className="border-t pt-4 space-y-3">
          <p className="text-sm text-muted-foreground font-medium">
            Or create a new connected account:
          </p>
          <div className="space-y-2">
            <Label htmlFor="connect-email">
              Email for New Connected Account
            </Label>
            <Input
              id="connect-email"
              type="email"
